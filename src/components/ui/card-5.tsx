...
export interface HighlightCardProps {
  ...
  iconColor?: string; // Color del icono del marcador; por defecto usa el del tema
  color?: keyof typeof colorThemes; // 'default' | 'blue' | 'violet' | 'orange' | 'gold' | 'naranja' | 'turquesa'
  badge?: React.ReactNode; // Elemento a la derecha, arriba de la línea divisoria
  className?: string;
}
...
export const HighlightCard = React.forwardRef<HTMLDivElement, HighlightCardProps>(
  ({ title, description, metricValue, metricLabel, buttonText, onButtonClick, icon, iconColor, color = "default", badge, className }, ref) => {
    ...
    return (
      ...
      >
        {/* Bookmark shape */}
      <div className="absolute top-0 right-6 h-16 w-12 bg-[#ead9c7] backdrop-blur-sm [clip-path:polygon(0%_0%,_100%_0%,_100%_100%,_50%_75%,_0%_100%)]">
        <div
          className="absolute inset-0 flex items-center justify-center"
          style={{ color: iconColor || 'var(--card-from-color)' }}
        >
          {icon}
        </div>
      </div>
        ...
        <div>
          <div className="flex items-start justify-between gap-4">
            <div className="min-w-0">
              <motion.h3 variants={itemVariants} className="text-2xl font-bold tracking-tight sm:text-3xl">
                {title}
              </motion.h3>
              <motion.p variants={itemVariants} className="mt-1 text-sm opacity-90 max-w-[85%] md:max-w-none">
                {description}
              </motion.p>
            </div>
            {badge && (
              <motion.div variants={itemVariants} className="shrink-0 pt-11 sm:pt-9">
                {badge}
              </motion.div>
            )}
          </div>
          ...
          <div className="flex items-end justify-between">
            <motion.div variants={itemVariants}>
              <p className="text-4xl font-bold tracking-tighter sm:text-5xl">{metricValue}</p>
