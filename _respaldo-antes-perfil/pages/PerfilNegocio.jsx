...
export default function PerfilNegocio() {
  ...
  // Lo apaga el dueño desde Ajustes > Mi perfil público: mientras
  // esté activo, la ficha dice "Cerrado temporalmente" y no promete
  // algo que el negocio no puede cumplir.
  const cerradoTemporal = useStore((s) => s.configuraciones.negocio?.cerradoTemporal)

  // La ficha que se ve es la de la sucursal activa: cada sucursal
  // tiene su propio nombre, dirección y teléfono. La foto y el resto
  // de los datos (RUC, correo...) son de la cuenta y no cambian.
      ...
      <PerfilHero
        ...
        chips={[
          ...
          ...(estadoVerificacion === 'aprobada'
            ...
            : []),
          ...(cerradoTemporal
            ? [{ label: 'Cerrado temporalmente', icono: 'pause' }]
                ...
                {idsConectadas.map((id) => (
                  <li key={id} className="pf-red">
                    <span className="pf-red-icono" style={{ background: REDES[id].color }}>
                      <IconRed nombre={id} size={15} style={{ color: '#ead9c7' }} />
                    </span>
