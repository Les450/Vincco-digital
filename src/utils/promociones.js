...
/* ¿Quién publicó esto: un negocio o un proveedor?

   Importa porque el Home no es el mismo para todos. El cliente ve lo
   que publican los negocios; el negocio ve lo que publican los
   proveedores, que es a quien le compra. La misma pantalla, la misma
   tarjeta, distinta vitrina.

   Lo publicado desde ahora lo dice solo (`negocio.rol`). Para lo que
   ya estaba guardado antes de ese cambio se deduce del id de la
   sucursal, que es lo único que quedó: las del negocio empiezan con
   "n" y las del proveedor con "p".                                */
export function rolDePublicacion(pub, clave = '') {
  if (pub?.negocio?.rol) return pub.negocio.rol

  const sucursal = String(clave).split(':')[1] || ''
  if (sucursal.startsWith('p')) return 'proveedor'
  if (sucursal.startsWith('n')) return 'negocio'

  // Sin pistas se asume negocio: es lo que había antes de que
  // existieran las publicaciones de proveedor.
  return 'negocio'
}

/* Si el proveedor que publicó ya es uno de los proveedores asociados
   del negocio que está mirando.

   Se compara por nombre porque hoy es lo único que hay: la lista de
   asociados vive en data_falso y no guarda el id de la sucursal que
   publica. Cuando la asociación se modele de verdad, esto pasa a ser
   una comparación de ids y se borra la normalización de texto.    */
export function esAsociado(negocio, asociados = []) {
  if (!negocio?.nombre) return false

  const limpiar = (t) => String(t).trim().toLowerCase()
  const nombre = limpiar(negocio.nombre)

  return asociados.some(
    (a) => a.estado === 'aceptada' && limpiar(a.nombre) === nombre
  )
}
...
function listar({ prefijo, clave, filtro, mapear, rolPublicador }) {
  const claves = clave ? [clave] : clavesDe(prefijo)

  return claves
    // Se arrastra la clave junto a cada publicación porque es lo que
    // permite deducir el rol de las publicaciones viejas.
    .flatMap((k) => leerPublicaciones(k).map((pub) => ({ pub, clave: k })))
    .filter(({ pub, clave: k }) =>
      (!rolPublicador || rolDePublicacion(pub, k) === rolPublicador) &&
      (!filtro || filtro(pub))
    )
    .map(({ pub }) => mapear(pub))
    .sort(porMasNuevas)
...
/* Los datos de ejemplo de data_falso son de comercios para
   consumidor final ("2x puntos hoy en La Cocina Nica"). Sirven para
   que el Home del cliente no se vea vacío antes de que un negocio
   publique, pero no tienen sentido en el Home de un negocio que está
   buscando a quién comprarle. Por eso solo se usan cuando no se está
   filtrando por proveedor.                                        */
const admiteEjemplos = (rolPublicador) => rolPublicador !== 'proveedor'

export function promocionesVisibles({ clave = null, respaldo = null, limitadas = false, rolPublicador = null } = {}) {
  ...
  if (limitadas || !admiteEjemplos(rolPublicador)) return []
  return promocionesEjemplo.map(normalizarPromocionEjemplo)
}

export function productosVisibles({ clave = null, respaldo = null, rolPublicador = null } = {}) {
  return listar({
...
export function destacadasVisibles({ clave = null, respaldo = null, rolPublicador = null } = {}) {
  const publicadas = listar({
  ...
  if (publicadas.length > 0) return publicadas
  if (!admiteEjemplos(rolPublicador)) return []
