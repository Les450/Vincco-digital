...
function usePublicaciones(tipo) {
  ...
  const respaldo = useNegocioRespaldo()
  const esSocio = userType === 'negocio' || userType === 'proveedor'

  const [items, setItems] = useState([])

  useEffect(() => {
    // null = todas las sucursales de todos los negocios (cliente)
    const clave = (base) => (esSocio ? (sucursal ? `${base}:${sucursal}` : base) : null)
    ...
    if (tipo === 'promocion' || tipo === 'limitada') {
      ...
      setItems(productosVisibles({ clave: clave(CLAVE_PRODUCTOS), respaldo }))
    } else {
      setItems(destacadasVisibles({ clave: clave(CLAVE_DESTACADAS), respaldo }))
    }
  }, [tipo, esSocio, sucursal, respaldo])
...
function SeccionPromociones() {
  return (
    <CarruselHome
      eyebrow="OFERTAS QUE TE ENCANTARÁN"
      titulo="Promociones locales"
      items={usePublicaciones('promocion')}
    />
  )
}
...
function SeccionProductos() {
  return (
    <CarruselHome
      ...
      items={usePublicaciones('producto')}
...
function SeccionLimitadas() {
  return (
    <CarruselHome
      ...
      titulo="Promociones limitadas"
      items={usePublicaciones('limitada')}
    />
  )
}
